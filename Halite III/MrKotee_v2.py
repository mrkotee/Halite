...
def scan_for_possibility_drop(game_map):
    height = game_map.height
    width = game_map.width
    positions_to_create_drop = {}
    for y in range(0, height, 2):
        for x in range(0, width, 2):
            halite_around = sum(halite_in_cells_around_pos(game_map, Position(x, y), 4).values())
            if halite_around >= constants.MAX_HALITE * 25:
                positions_to_create_drop[x, y] = halite_around
    return positions_to_create_drop
...
NEED_DROP = False
first_drop_pos = None
first_ship_id_to_create = None
halite_on_map = 0
positions_to_dropoffs = {}
HALITE_FOR_PLAYER = 0
...
while True:
    ...
    if game.turn_number == 1:
        ...
        positions_to_dropoffs = scan_for_possibility_drop(game_map)
        logging.info(f'positions to dropoffs {len(positions_to_dropoffs)}')
        halite_around_shipyard = sum(halite_in_cells_around_pos(game_map, me.shipyard.position, 4).values())
        if halite_around_shipyard <= constants.MAX_HALITE * 20:
            NEED_DROP = True
            logging.info(f'need drop {NEED_DROP}')

    if game.turn_number == 2:
        nearest_pos_to_drop = []
        for drop_coords in positions_to_dropoffs:
            distance = game_map.calculate_distance(me.shipyard.position, Position(drop_coords[0], drop_coords[1]))
            if 8 <= distance <= 12:
                nearest_pos_to_drop.append(drop_coords)

        max_halite = 0
        for drop_coords in nearest_pos_to_drop:
            if positions_to_dropoffs[drop_coords] > max_halite:
                max_halite = positions_to_dropoffs[drop_coords]
                first_drop_pos = Position(drop_coords[0], drop_coords[1])
        logging.info(f"pos to first drop: {first_drop_pos}")
    ...
    for player in players:
        for ship in player.get_ships():
            ...
            if ship.halite_amount >= constants.MAX_HALITE * .5:  # \and game_map.calculate_distance(ship.position,
    ...
    if first_ship_id_to_create:
        if not me.get_ship(first_ship_id_to_create):
            first_ship_id_to_create = None
        else:
            not_full_ships.remove(me.get_ship(first_ship_id_to_create))

    if first_drop_pos and not first_ship_id_to_create and 3 < game.turn_number <= 15 and me.halite_amount >= MIN_HALITE:
        closest_ship = None
        shortest_distance = 12
        for ship in my_ships:
            distance = game_map.calculate_distance(ship.position, first_drop_pos)
            if distance < shortest_distance:
                closest_ship = ship
                shortest_distance = distance
        first_ship_id_to_create = closest_ship.id
        not_full_ships.remove(closest_ship)
        if closest_ship.id in full_ships_id:
            full_ships_id.remove(closest_ship.id)

    if first_ship_id_to_create:
        ship = me.get_ship(first_ship_id_to_create)
        if ship.position != first_drop_pos:
            directions_dict = get_directions_pos(game_map, ship)
            for direction in directions_dict.copy():
                ...
                if (target_cell in will_occuped_cells or target_cell in occuped_cells) and direction != Direction.Still:
                    directions_dict.pop(direction)
            command_queue.append(ship_move(game_map, ship, first_drop_pos, directions_dict, will_occuped_cells))
        else:
            command_queue.append(ship.make_dropoff())
            logging.info("ship ID {} creates dropoff at pos {}".format(first_ship_id_to_create, ship.position))
    ...
    for ship in not_full_ships:
        ...
        if ship.position == me.shipyard.position:
            ...
            if len(my_ships) >= 10 and ship.halite_amount <= constants.MAX_HALITE / 9 and distance_to_enemy <= 4:
                ...
                priority_cells = {}
                for cell_cords, halite_amount in halite_in_cells_around_pos(game_map, ship.position, 5).items():
